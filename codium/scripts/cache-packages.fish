...
if test -n "$t";
    printf "%s\n" $t | xargs -I {} nix build --print-out-paths .#{} > $PATHS_FOR_PACKAGES/tmp
    cat $PATHS_FOR_PACKAGES/tmp | xargs -I {} nix-store -qR {} | cachix-wrapped push $CACHIX_CACHE;
end

rm -rf $PATHS_FOR_PACKAGES